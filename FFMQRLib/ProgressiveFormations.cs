	...
	public partial class FFMQRom : SnesRom
	{
		public void ProgressiveFormation(ProgressiveFormationsModes formationmode, Overworld overworld, MT19337 rng)
		{
			...
			List<(EnemyCategory, List<byte>)> formationsList = new()
			{
				...
				(EnemyCategory.Mummy, new() { 0x55, 0x55, 0x50, 0x7F }),
				(EnemyCategory.Hag, new() { 0x32, 0x35, 0x93, 0x93 }),
				(EnemyCategory.Ninja, new() { 0x62, 0x62, 0x66, 0xBE }),
				(EnemyCategory.Sphinx, new() { 0x47, 0x4B, 0x9C, 0xA4 }),
				(EnemyCategory.Centaur, new() { 0x28, 0x28, 0x5C, 0x5C }),
				(EnemyCategory.FatBird, new() { 0x3F, 0x42, 0x65, 0x65 }),
				(EnemyCategory.Ghost, new() { 0x58, 0x58, 0x58, 0x80 }),
				(EnemyCategory.Eye, new() { 0x46, 0x4A, 0x9B, 0x9B }),
				(EnemyCategory.Bat, new() { 0x6A, 0x71, 0x71, 0x94 }),
				(EnemyCategory.Mage, new() { 0x3C, 0x3E, 0xA5, 0xA8 }),
				(EnemyCategory.Turtle, new() { 0x2C, 0x2C, 0x75, 0x78 }),
				(EnemyCategory.Scorpion, new() { 0x22, 0x22, 0x85, 0x88 }),
				(EnemyCategory.Doggo, new() { 0x5D, 0x5D, 0x60, 0xC6 }),
				(EnemyCategory.Edgehog, new() { 0x23, 0x23, 0x2B, 0x2B }),
				(EnemyCategory.Lamia, new() { 0x37, 0x3A, 0xA9, 0xB5 }),
				(EnemyCategory.Devil, new() { 0x63, 0x63, 0x67, 0xBB }),
				(EnemyCategory.Minotaur, new() { 0x20, 0x20, 0x20, 0x20 }),
				(EnemyCategory.Behemot, new() { 0x1E, 0x1E, 0x1E, 0x1E }),
				(EnemyCategory.Squid, new() { 0x40, 0x48, 0x48, 0x48 }),
				(EnemyCategory.Crab, new() { 0x44, 0x4C, 0x4C, 0x4C }),
